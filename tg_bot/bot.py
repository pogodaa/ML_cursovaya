...
# try:
#     model = joblib.load('models/best_model_lightgbm.pkl')  # ИСПРАВЛЕНО имя файла
#     with open('models/feature_names.json', 'r', encoding='utf-8') as f:
#         FEATURE_NAMES = json.load(f)
#     print(f"✅ Модель загружена. Ожидает {len(FEATURE_NAMES)} признаков")
# except Exception as e:
...
# Загружаем модель и признаки - ИСПРАВЛЕННЫЕ ПУТИ
try:
    model = joblib.load('models/best_model_lightgbm_clean.pkl')  # Новая модель без утечек
    with open('models/feature_names_clean.json', 'r', encoding='utf-8') as f:
        FEATURE_NAMES = json.load(f)
    print(f"✅ Чистая модель загружена. Ожидает {len(FEATURE_NAMES)} признаков")
except Exception as e:
...
def create_realistic_features(hour, day_of_week, month, target_date):
    ...
    # 11. ДОБАВЛЯЕМ НЕДОСТАЮЩИЕ ПРИЗНАКИ ИЗ feature_names.json
    features['winter_evening_peak'] = 1 if (month in [12, 1, 2] and 18 <= hour <= 22) else 0
    features['summer_afternoon_ac'] = 1 if (month in [6, 7, 8] and 10 <= hour <= 16) else 0
    features['weekend_morning_kitchen'] = 1 if (day_of_week >= 5 and 7 <= hour <= 9) else 0
    
    features['extreme_evening_peak'] = 1 if 19 <= hour <= 21 else 0
    features['sunday_evening_surge'] = 1 if (day_of_week == 6 and 18 <= hour <= 22) else 0
    features['deep_night_low'] = 1 if 1 <= hour <= 4 else 0
    
    # Взаимодействия с суб-счетчиками
    submetering = data_gen.get_realistic_submetering(hour, day_of_week, month)
    features['evening_ac_demand'] = features['is_evening_peak'] * submetering['ac_heating_ratio']
    features['morning_kitchen_demand'] = features['is_morning_peak'] * submetering['kitchen_ratio']
    
    features['winter_evening_heating'] = 1 if (month in [12, 1, 2] and 18 <= hour <= 22 and submetering['ac_heating_ratio'] > 0.3) else 0
    features['summer_afternoon_cooling'] = 1 if (month in [6, 7, 8] and 10 <= hour <= 16 and submetering['ac_heating_ratio'] > 0.4) else 0
